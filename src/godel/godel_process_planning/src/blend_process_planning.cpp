...
descartes_core::TrajectoryPtPtr toDescartesBlendPt(const Eigen::Affine3d& pose, double dt)
{
  ...
  // By specifying a range of 360 degrees and a step of 360 degrees, the system will sample
  // at the nominal pose +/- 180 degrees for scanning purposes.
  TolerancedFrame frame(
      pose, ToleranceBase::zeroTolerance<PositionTolerance>(x, y, z),
      ToleranceBase::createSymmetric<OrientationTolerance>(rx, ry, rz, 0, 0, 2*M_PI));

  return TrajectoryPtPtr(new CartTrajectoryPt(frame, 0.0, 2*M_PI, tm));
}*/
